...
local function valueToString(value, depth)
	...
	if t == "table" then
		...
		for k, v in pairs(value) do
			str =
				str ..
				string.rep("  ", depth + 1) .. "[" .. valueToString(k) .. "]" .. " = " .. valueToString(v, depth + 1) .. ",\n"
		end
...
local np_pat1, np_pat2 =
	("[^SEP:]+SEP%.%.SEP?"):gsub("SEP", directorySeperator),
	("SEP+%.?SEP"):gsub("SEP", directorySeperator)
...
function handlers.stackTrace(req)
	...
	local lastFrame =
		(req.arguments.levels and (req.arguments.levels ~= 0)) and (firstFrame + req.arguments.levels - 1) or (9999)
