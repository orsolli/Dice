...
Script.knobwidth=12
Script.style = 0--"Scrollbar"--Style.Slider.Horizontal
--Script.layout = "Horizontal"
Script.offset=0

--Style constants declared in engine
--[[
SLIDER_SCROLLBAR
SLIDER_TRACKBAR
SLIDER_STEPPER
...
function Script:Draw(x,y,width,height)
	...
	if self.And(self.style,SLIDER_VERTICAL) then--self.layout=="Vertical" then--self:bitand(style,Style.Slider.Horizontal)==false then
		
		-----------------------------------------------
		--Draw vertical slider
		-----------------------------------------------
		
		--if self.styleTrackbar" then
		if self.And(self.style,SLIDER_TRACKBAR)==true then
		...
		elseif self.And(self.style,SLIDER_STEPPER) then
		...
		if self.And(self.style,SLIDER_TRACKBAR) then
...
function Script:GetKnobArea()
	...
	local sz = self.widget:GetSize(true)
	local style = self.style--self.widget:GetStyle()
	if self.And(self.style,SLIDER_VERTICAL) then	
	...
	if self.knobgrabbed==true then
		...
		if x>=0 and x<sz.width and y>=0 and y<sz.height then
			...
			--if self.layout=="Vertical" then--self:bitand(style,Style.Slider.Horizontal)==false then
				...
				if self.And(self.style,SLIDER_STEPPER) then
		...
		if x>0 and y>0 and x<sz.width and y<sz.height then
			...
			--if self.layout=="Vertical" then--self:bitand(style,Style.Slider.Horizontal)==false then
				...
				if self.And(self.style,SLIDER_STEPPER) then
					...
					return
				elseif self.And(self.style,SLIDER_TRACKBAR) then
