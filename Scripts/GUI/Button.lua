...
function Script:Cancel_()
	EventQueue:Emit(Event.WidgetAction,self.widget)
end

function Script:UpdateStyle()
	if self.And(self.style,BUTTON_OK) then
		self.OK = self.OK_
	else
		self.OK = nil
	end
	if self.And(self.style,BUTTON_CANCEL) then
		self.Cancel = self.Cancel_
	else
		self.Cancel = nil
	end
end
...
function Script:Draw(x,y,width,height)
	...
	if self.And(self.style,BUTTON_LINK) then
		--Draw text
		if text~="" then
	...
	--[[elseif self.And(self.style,BUTTON_LINK)==false then
		--Draw text
		if text~="" then
			--gui:SetColor(0,0,0,0)		
			--gui:Clear()-- clear the alpha channel
	...
	elseif self.And(self.style,BUTTON_CHECKBOX) then
...
function Script:MouseUp(button,x,y)
	if button==Mouse.Left then
		...
		self.pushed=false
		if self.hovered then
			if self.And(self.style,BUTTON_CHECKBOX) then
