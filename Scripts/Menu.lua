function BuildMenu(context)
	...
	GameMenu.resumebutton = Widget:Button("RESUME GAME",100,gui:GetBase():GetSize().y/2-60,300,20,gui:GetBase())
	GameMenu.resumebutton:SetStyle(BUTTON_LINK + BUTTON_CANCEL)
	GameMenu.resumebutton:SetAlignment(1,0,0,0)
	GameMenu.resumebutton:Hide()
	...
	GameMenu.applyoptions:SetStyle(BUTTON_OK)	
	GameMenu.closeoptions:SetStyle(BUTTON_CANCEL)	
	...
	GameMenu.tfilter:SetStyle(BUTTON_CHECKBOX)
	y=y+sep
	
	--Create a checkbox
	GameMenu.vsync = Widget:Button("Vertical Sync",20,y,200,30,panel)
	GameMenu.vsync:SetStyle(BUTTON_CHECKBOX)
	y=y+sep
	...
	GameMenu.confirmquit:SetStyle(BUTTON_OK)	
	GameMenu.cancelquit:SetStyle(BUTTON_CANCEL)	
	...
	function GameMenu:ProcessEvent(event)
		...
		elseif event.id == Event.WidgetAction then
			...
			elseif event.source == self.confirmquit then
				self.context:GetWindow():Close()
				
			elseif event.source == self.resumebutton then
				self.gui:Hide()
				self.context:GetWindow():FlushMouse()
				self.context:GetWindow():FlushKeys()
				self.context:GetWindow():HideMouse()
