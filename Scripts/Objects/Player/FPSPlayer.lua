...
function Script:UpdateNetwork()
	if P2P:Connected()==true then
		if type(MESSAGE_UPDATEPLAYER)~="number" then
			return
		end
		...
		if tm - self.lastupdatenetworktime > 1000/30 then
			...
			P2P:Broadcast(MESSAGE_UPDATEPLAYER,stream)
			stream:Release()
		end
	end
end
...
function Script:UpdateWorld()
	...
	if SuspendInput==true then return end
				...
				if usableentity~=nil then
					...
					if self.carryingEntity == nil then
						...
						if mass>0 and mass<=self.maxcarryweight then
							...
							self.carryingEntity:SetSweptCollisionMode(false)
		...
		if self.entity:GetAirborne()==false then
			if (speed>self.moveSpeed*0.5) then
				...
				if t-self.lastfootsteptime>repeatdelay then
					...
					if self.sound.footsteps.concrete.step[index]~=nil then
...
function Script:UpdatePhysics()
	...
	if self.carryingEntity then
		...
		if diff>1.5 then
			...
			self.effector:SetTargetPosition(pos,1)
			self.effector:SetTargetRotation(rot,1)
